...
<html lang="en">
  ...
  <body>    
    ...
    <section id="imgBanner">
      <h2>Watchlist</h2>
      <h4><font color="white">Watchlist is a feature of Rig The Market.This faeture enables you to watch all your target,specific or desired shares together. This is a place where you just need to add shares and you can watch without searching in the list.</font></h4>
    </section>
    ...
    <div>
        ...
        <%
            ArrayList<Watchlist> sharelist;
            if(request.getAttribute("shares")!= null)
            {
                   sharelist = (ArrayList<Watchlist>) request.getAttribute("shares");
                   
                   for(Watchlist w : sharelist) {    
          %>
          <tr> 
              <td> <%= w.getId().getSymbol()%></td>
              <td><%= w.getId().getName()%></td>
              <td><%= w.getId().getPrice()%> </td>
              <td><%= w.getId().getPreviousopen() %></td>
              <td><%= w.getId().getPreviousclose()%></td>
              <td><%= w.getId().getLow() %></td>
              <td><%= w.getId().getHigh() %></td>
              <td><%= w.getId().getChange() %></td>
              <td><%= w.getId().getChangeinpercentage() %></td>
              <td><a href="RemoveWatchListServ?id=<%=w.getWlid() %>"><button class="buybutton button1">Remove</button></a></td>
